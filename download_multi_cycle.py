...
def obfuscate_content(content_bytes):
    # XOR and base64 using CalauanWeather2026 key
    key_bytes = XOR_KEY.encode('utf-8')
    obfuscated = bytearray(len(content_bytes))
    for i in range(len(content_bytes)):
        obfuscated[i] = content_bytes[i] ^ key_bytes[i % len(key_bytes)]
...
def main():
    ...
    try:
        latest_dt = get_latest_valid_run()
    except Exception as e:
    ...
    for dt in cycles:
        ...
        # 1. Base Tracks (.enc)
        base_tracks_fn = f"fnv3p2_{date_str}T{hour_str}_00.enc"
        base_tracks_url = f"{BASE_TRACKS_URL}/FNV3P2_{date_str}T{hour_str}_00_cyclogenesis.csv"
        
        # 2. Base Paired (.enc)
        base_paired_fn = f"fnv3p2_paired_{date_str}T{hour_str}_00.enc"
        ...
        # 3. Large Tracks (.enc)
        large_tracks_fn = f"fnv3_large_{date_str}T{hour_str}_00.enc"
        large_tracks_url = f"{LARGE_TRACKS_URL}/FNV3_LARGE_ENSEMBLE_{date_str}T{hour_str}_00_cyclogenesis.csv"
        
        # 4. Large Paired (.enc)
        large_paired_fn = f"fnv3_large_paired_{date_str}T{hour_str}_00.enc"
        ...
        # If this is the latest cycle, also update the _latest.enc files
        if dt == cycles[0]:
            import shutil
            if success_tracks:
                shutil.copy(os.path.join(DATA_DIR, base_tracks_fn), os.path.join(DATA_DIR, "fnv3p2_latest.enc"))
                base_paired_path = os.path.join(DATA_DIR, base_paired_fn)
                if os.path.exists(base_paired_path):
                    shutil.copy(base_paired_path, os.path.join(DATA_DIR, "fnv3p2_paired_latest.enc"))
                    print("Updated base latest files.")
            if success_large:
                shutil.copy(os.path.join(DATA_DIR, large_tracks_fn), os.path.join(DATA_DIR, "fnv3_large_latest.enc"))
                large_paired_path = os.path.join(DATA_DIR, large_paired_fn)
                if os.path.exists(large_paired_path):
                    shutil.copy(large_paired_path, os.path.join(DATA_DIR, "fnv3_large_paired_latest.enc"))
    ...
    # Clean up old ENC files that are no longer active to keep the repository clean
    ...
    for f in os.listdir(DATA_DIR):
        if f.startswith("fnv3") and f.endswith(".enc"):
            if f.endswith("_latest.enc"):
                continue
