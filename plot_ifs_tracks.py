...
def generate_plot(data, max_lead_time, output_file, title, runtime_text):
    ...
    plt.text(0.99, 0.01, legend_text, transform=ax.transAxes, fontsize=10, verticalalignment='bottom', horizontalalignment='right', bbox=dict(facecolor='#1e293b', alpha=0.9, edgecolor='#475569', boxstyle='round,pad=0.3'), color='#cbd5e1')
    
    # 2. Use global figure title configuration so it stays pinned within the safe canvas margins
    fig.suptitle(title, fontsize=14, weight='bold', color='#f8fafc', y=0.94)
    ...
    plt.savefig(output_file, dpi=300, facecolor='#0f172a', edgecolor='none')
...
if __name__ == "__main__":
    ...
    enc_file = "public/data/ifs_tc_latest.enc"
    ...
    title_5day = f"ECMWF IFS 5-Day Forecast Tropical Cyclone Tracks\nWestern Pacific ({forecast_start} to {end_5day})"
    ...
    # Dynamic "15-day" (max lead time) plot
    actual_max_lead_hours = int(data['lead_time_hours'].max())
    long_plot_days = actual_max_lead_hours // 24
    end_long = (latest_ph + timedelta(days=long_plot_days)).strftime("%Y-%m-%d")
    title_long = f"ECMWF IFS {long_plot_days}-Day Forecast Tropical Cyclone Tracks\nWestern Pacific ({forecast_start} to {end_long})"
    out_long = f"public/assets/ifs_tropical_cyclone_15day_forecast_{init_time_str}.png"
    generate_plot(data, actual_max_lead_hours, out_long, title_long, runtime_text)
