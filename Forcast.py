...
def obfuscate_and_save(input_path, output_path):
    ...
    try:
        ...
        for i in range(len(content_bytes)):
            xored[i] = content_bytes[i] ^ key_bytes[i % len(key_bytes)]
...
def process_and_plot(model, latest_url, date_str, hour_str, is_base_model):
    ...
    # Copy model-specific CSV and ENC to public/data/
    os.makedirs("public/data", exist_ok=True)
    model_lower = model.lower()
    
    # Save cyclogenesis CSV and ENC
    shutil.copy(local_csv, f"public/data/{model_lower}_latest.csv")
    obfuscate_and_save(local_csv, f"public/data/{model_lower}_latest.enc")
    print(f"Saved model-specific latest CSV and ENC for {model}")
    ...
    try:
        ...
        # Save paired CSV and ENC
        shutil.copy(paired_local_csv, f"public/data/{model_lower}_paired_latest.csv")
        obfuscate_and_save(paired_local_csv, f"public/data/{model_lower}_paired_latest.enc")
        print(f"Saved model-specific paired latest CSV and ENC for {model}")
    except Exception as e:
    ...
    ax.text(
        118, 13, 'West Philippine\nSea', fontsize=8, color='#94a3b8', weight='bold',
    ...
    par_patch = PathPatch(par_path, edgecolor='#ef4444', linestyle='-', linewidth=2, facecolor='none', transform=ccrs.PlateCarree())
            ...
            for sample in track_data['sample'].unique():
                ...
                for i in range(len(lons)):
                    ...
                    if color is None:
                        continue
                    # Shadow
                    ax.plot(lons[i], lats[i], color='#000000', marker='o', markersize=8,
                            markeredgewidth=0, alpha=0.2, transform=ccrs.PlateCarree(), zorder=3)
    ...
    legend = ax.legend(
        ...
        frameon=False, fontsize=10, labelcolor='#e2e8f0'
    )
