...
for mode in ['standard', 'cluster']:
    ...
    ax.set_extent([-75, 0, 0, 40], crs=projection)
    ...
    try:
        ...
        if found_geojson_path:
            with open(found_geojson_path, 'r', encoding='utf-8') as geojson_file_handle:
                geojson_content_dict = json.load(geojson_file_handle)
            province_shapely_geometries = [shape(prov_feat['geometry']) for prov_feat in geojson_content_dict['features']]
            ax.add_geometries(province_shapely_geometries, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.4, zorder=3)
    except Exception as province_load_error:
        print(f"Warning: Failed to overlay province boundaries: {province_load_error}")
        
    gl = ax.gridlines(draw_labels=True, linewidth=0.5, color='#334155', alpha=0.8, linestyle='--')
    gl.xlocator = plt.FixedLocator(np.arange(105, 181, 15))
    gl.ylocator = plt.FixedLocator(np.arange(0, 41, 5))
    ...
    ax.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', 
    ...
    if mode == 'standard':
        ...
        legend = ax.legend(
            ...
            frameon=True, facecolor='#1e293b', framealpha=0.85, edgecolor='#334155',
            title="Ensemble Cluster Support", title_fontsize=9, fontsize=8, labelcolor='#e2e8f0'
        )
