...
def generate_empty_state():
    ...
    fig, (ax_head, ax) = plt.subplots(2, 1, figsize=(12, 13.5), facecolor='#0f172a', gridspec_kw={'height_ratios': [0.08, 0.92], 'hspace': 0.02})
    
    ax_head.set_axis_off()
    ax_head.text(0.02, 0.4, "NO ACTIVE TROPICAL CYCLONES", fontsize=20, fontweight='900', color='#f8fafc', va='center')
    ...
    ax.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', linestyle='-', linewidth=2.5, alpha=0.8, transform=ccrs.PlateCarree()))
...
def main():
    ...
    for latest_pt in latest_pts:
        ...
        try:
            ...
            if os.path.exists(prov_path):
                ...
                ax.add_geometries(province_shapely_geometries, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.5, zorder=3)
        except Exception as e:
        ...
        ax.text(118, 13, 'West Philippine\nSea', fontsize=5, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        ax.text(130, 20, 'Philippine\nSea', fontsize=9, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        
        # Watermark text at the bottom right corner
        ax.text(0.98, 0.02, 'Calauan Weather', transform=ax.transAxes, fontsize=10, color='#cbd5e1', weight='bold', style='italic', ha='right', va='bottom', alpha=0.45, zorder=10)
        ...
        ax.plot(lons, lats, color='#64748b', linewidth=2.5, linestyle='-', zorder=4, transform=ccrs.PlateCarree())
        ...
        try:
            ...
            lbl_latest = f"{latest_pt['cycle']} (LATEST)"
            
        # Orientation of indicator text
        ha = 'left' if latest_pt['lon'] < 135 else 'right'
        va = 'bottom' if latest_pt['lat'] < 30 else 'top'
        dist_offset = 1.0 if ha == 'left' else -1.0
        
        ax.text(
            latest_pt['lon'] + dist_offset, latest_pt['lat'] + dist_offset,
            lbl_latest,
            ...
            ha=ha,
            va=va,
            bbox=dict(facecolor='#0f172a', alpha=0.85, edgecolor='#334155', boxstyle='round,pad=0.25', linewidth=0.8),
            zorder=8
        )
        
        legend_elements = [
            plt.Line2D([0], [0], color='#ef4444', linewidth=2.2, label='PAR Boundary'),
            plt.Line2D([0], [0], color='#64748b', linewidth=2.5, label='Track Line'),
        ...
        ax.legend(handles=legend_elements, loc='upper left', bbox_to_anchor=(0.02, 0.98), frameon=True, facecolor='#1e293b', edgecolor='#334155', labelcolor='#e2e8f0', framealpha=0.9, fontsize=8, ncol=2)
        
        output_file_storm = os.path.join(DATA_DIR, "..", "assets", f"tc_positions_{latest_tid}.png")
        plt.savefig(output_file_storm, dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor(), edgecolor='none')
        ...
        if latest_df is not None:
            ...
            if not storm_df.empty:
                ...
                ax_f.add_feature(cfeature.LAND, facecolor='#1e293b')
                ax_f.add_feature(cfeature.COASTLINE, edgecolor='#475569', linewidth=1.5)
                ax_f.add_feature(cfeature.BORDERS, edgecolor='#475569', linestyle=':', linewidth=1)
                ax_f.add_feature(cfeature.OCEAN, facecolor='#0f172a')
                ...
                try:
                    ...
                    if os.path.exists(prov_path):
                        ...
                        ax_f.add_geometries(province_geoms, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.5, zorder=3)
                except Exception as e:
                    print(f"Warning: Failed to load provinces for forecast map: {e}")

                gl_f = ax_f.gridlines(draw_labels=True, linewidth=0.5, color='#334155', alpha=0.8, linestyle='--')
                gl_f.xlocator = plt.FixedLocator(np.arange(105, 156, 5))
                gl_f.ylocator = plt.FixedLocator(np.arange(0, 41, 5))
                gl_f.xlabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
                gl_f.ylabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
                ...
                # Watermark
                ax_f.text(0.98, 0.02, 'Calauan Weather', transform=ax_f.transAxes, fontsize=10, color='#cbd5e1', weight='bold', style='italic', ha='right', va='bottom', alpha=0.45, zorder=10)
                
                # PAR boundary
                ax_f.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', linestyle='-', linewidth=2.5, alpha=0.8, transform=ccrs.PlateCarree()))
                ...
                if cone_coords:
                    ...
                    # Fill the cone
                    ax_f.fill(cone_lons, cone_lats, color='white', alpha=0.15, transform=ccrs.PlateCarree(), zorder=3)
                    # Border of the cone
                    ax_f.plot(cone_lons, cone_lats, color='white', linestyle='--', linewidth=1.2, alpha=0.5, transform=ccrs.PlateCarree(), zorder=3)
                ...
                if len(hist_lats) > 1:
                    ax_f.plot(hist_lons, hist_lats, color='#64748b', linewidth=2.0, linestyle='--', zorder=4, transform=ccrs.PlateCarree())
                    for pt in merged_history[:-1]:
                        ax_f.plot(pt['lon'], pt['lat'], marker='o', color='#cbd5e1', markersize=5, markeredgecolor='#64748b', markeredgewidth=0.8, zorder=5, transform=ccrs.PlateCarree())

                # Plot Forecast Track Line
                ax_f.plot(f_lons, f_lats, color='#cbd5e1', linewidth=2.5, linestyle='-', zorder=4, transform=ccrs.PlateCarree())
                ...
                for i, (lat, lon, lead, wind, press) in enumerate(zip(f_lats, f_lons, f_leads, f_winds, f_pressures)):
                    ...
                    if lead in [0, 24, 48, 72, 96, 120, 144] or (i == len(f_lats) - 1 and lead not in [0, 24, 48, 72, 96, 120, 144]):
                        ...
                        dx_f = 1.0 if lon < 135 else -1.0
                        dy_f = 1.0 if lat < 30 else -1.0
                        ...
                        ax_f.text(
                            ...
                            ha=ha_f,
                            va=va_f,
                            bbox=dict(facecolor='#0f172a', alpha=0.85, edgecolor='#334155', boxstyle='round,pad=0.2', linewidth=0.6),
                            zorder=8
                        )
                ...
                legend_elements_f = [
                    plt.Line2D([0], [0], color='#ef4444', linewidth=2.2, label='PAR Boundary'),
                    plt.Line2D([0], [0], color='#cbd5e1', linewidth=2.5, label='Forecast Track'),
                    plt.Line2D([0], [0], color='#64748b', linewidth=2.0, linestyle='--', label='Past Track (History)'),
                    mpatches.Patch(facecolor='white', edgecolor='white', linestyle='--', alpha=0.2, label='Cone of Uncertainty'),
                    plt.Line2D([0], [0], marker='o', color='none', markerfacecolor='#FF007F', markeredgecolor='white', markersize=6, label='Super Typhoon'),
                    plt.Line2D([0], [0], marker='o', color='none', markerfacecolor='#A83232', markeredgecolor='white', markersize=6, label='Typhoon'),
                    plt.Line2D([0], [0], marker='o', color='none', markerfacecolor='#E67E22', markeredgecolor='white', markersize=6, label='Severe Tropical Storm'),
                    plt.Line2D([0], [0], marker='o', color='none', markerfacecolor='#F1C40F', markeredgecolor='white', markersize=6, label='Tropical Storm'),
                ...
                ax_f.legend(handles=legend_elements_f, loc='upper left', bbox_to_anchor=(0.02, 0.98), frameon=True, facecolor='#1e293b', edgecolor='#334155', labelcolor='#e2e8f0', framealpha=0.9, fontsize=8, ncol=2)
                output_file_storm_f = os.path.join(DATA_DIR, "..", "assets", f"tc_forecast_{latest_tid}.png")
                plt.savefig(output_file_storm_f, dpi=300, bbox_inches='tight', facecolor=fig_f.get_facecolor(), edgecolor='none')
                plt.close()
    ...
    if latest_pts:
        ...
        ax_all.set_facecolor('#0f172a')
        ax_all.add_feature(cfeature.LAND, facecolor='#1e293b')
        ax_all.add_feature(cfeature.COASTLINE, edgecolor='#475569', linewidth=1.5)
        ax_all.add_feature(cfeature.BORDERS, edgecolor='#475569', linestyle=':', linewidth=1)
        ax_all.add_feature(cfeature.OCEAN, facecolor='#0f172a')
        ...
        try:
            ...
            if os.path.exists(prov_path):
                ...
                ax_all.add_geometries(province_shapely_geometries, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.5, zorder=3)
        except Exception as e:
            print(f"Warning: Failed to load province boundaries: {e}")

        gl_all = ax_all.gridlines(draw_labels=True, linewidth=0.5, color='#334155', alpha=0.8, linestyle='--')
        gl_all.xlocator = plt.FixedLocator(np.arange(105, 156, 5))
        gl_all.ylocator = plt.FixedLocator(np.arange(0, 41, 5))
        gl_all.xlabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
        gl_all.ylabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
        gl_all.top_labels = False
        gl_all.right_labels = False
        
        ax_all.text(118, 13, 'West Philippine\nSea', fontsize=5, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        ax_all.text(130, 20, 'Philippine\nSea', fontsize=9, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        ax_all.text(0.98, 0.02, 'Calauan Weather', transform=ax_all.transAxes, fontsize=10, color='#cbd5e1', weight='bold', style='italic', ha='right', va='bottom', alpha=0.45, zorder=10)
        ax_all.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', linestyle='-', linewidth=2.5, alpha=0.8, transform=ccrs.PlateCarree()))
        ...
        for latest_pt in latest_pts:
            ...
            ax_all.plot(lons, lats, color='#64748b', linewidth=2.5, linestyle='-', zorder=4, transform=ccrs.PlateCarree())
            ...
            ax_all.text(
                latest_pt['lon'] + 1.2, latest_pt['lat'] + 1.2,
                s_name,
                transform=ccrs.PlateCarree(),
                fontsize=8,
                color='white',
        ...
        ax_all.legend(handles=legend_elements_all, loc='upper left', bbox_to_anchor=(0.02, 0.98), frameon=True, facecolor='#1e293b', edgecolor='#334155', labelcolor='#e2e8f0', framealpha=0.9, fontsize=8, ncol=2)
        
        output_file_latest = os.path.join(DATA_DIR, "..", "assets", "tc_positions_latest.png")
        plt.savefig(output_file_latest, dpi=300, bbox_inches='tight', facecolor=fig_all.get_facecolor(), edgecolor='none')
        plt.close()
    ...
    if latest_pts and latest_df is not None:
        ...
        ax_f_all.set_facecolor('#0f172a')
        ax_f_all.add_feature(cfeature.LAND, facecolor='#1e293b')
        ax_f_all.add_feature(cfeature.COASTLINE, edgecolor='#475569', linewidth=1.5)
        ax_f_all.add_feature(cfeature.BORDERS, edgecolor='#475569', linestyle=':', linewidth=1)
        ax_f_all.add_feature(cfeature.OCEAN, facecolor='#0f172a')
        ...
        try:
            ...
            if os.path.exists(prov_path):
                with open(prov_path, 'r', encoding='utf-8') as f_prov:
                    geojson_prov = json.load(f_prov)
                province_shapely_geometries = [shape(prov_feat['geometry']) for prov_feat in geojson_prov['features']]
                ax_f_all.add_geometries(province_shapely_geometries, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.5, zorder=3)
        except:
            pass

        gl_f_all = ax_f_all.gridlines(draw_labels=True, linewidth=0.5, color='#334155', alpha=0.8, linestyle='--')
        gl_f_all.xlocator = plt.FixedLocator(np.arange(105, 156, 5))
        gl_f_all.ylocator = plt.FixedLocator(np.arange(0, 41, 5))
        gl_f_all.xlabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
        gl_f_all.ylabel_style = {'size': 12, 'weight': 'bold', 'color': '#cbd5e1'}
        gl_f_all.top_labels = False
        gl_f_all.right_labels = False
        
        ax_f_all.text(118, 13, 'West Philippine\nSea', fontsize=5, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        ax_f_all.text(130, 20, 'Philippine\nSea', fontsize=9, color='#cbd5e1', weight='bold', transform=ccrs.PlateCarree(), ha='center', va='center', style='italic', alpha=0.3)
        ax_f_all.text(0.98, 0.02, 'Calauan Weather', transform=ax_f_all.transAxes, fontsize=10, color='#cbd5e1', weight='bold', style='italic', ha='right', va='bottom', alpha=0.45, zorder=10)
        ax_f_all.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', linestyle='-', linewidth=2.5, alpha=0.8, transform=ccrs.PlateCarree()))
        ...
        for latest_pt in latest_pts:
            ...
            if not storm_df.empty:
                ...
                if len(hist_lats) > 1:
                    ax_f_all.plot(hist_lons, hist_lats, color='#64748b', linewidth=2.0, linestyle='--', zorder=4, transform=ccrs.PlateCarree())
                    for pt in track_pts_all[:-1]:
                        ax_f_all.plot(pt['lon'], pt['lat'], marker='o', color='#cbd5e1', markersize=5, markeredgecolor='#64748b', markeredgewidth=0.8, zorder=5, transform=ccrs.PlateCarree())
                
                # Plot Forecast Line
                ax_f_all.plot(f_lons, f_lats, color='#cbd5e1', linewidth=2.5, linestyle='-', zorder=4, transform=ccrs.PlateCarree())
        ...
        ax_f_all.legend(handles=legend_elements_f_all, loc='upper left', bbox_to_anchor=(0.02, 0.98), frameon=True, facecolor='#1e293b', edgecolor='#334155', labelcolor='#e2e8f0', framealpha=0.9, fontsize=8, ncol=2)
        
        output_file_latest_f = os.path.join(DATA_DIR, "..", "assets", "tc_forecast_latest.png")
        plt.savefig(output_file_latest_f, dpi=300, bbox_inches='tight', facecolor=fig_f_all.get_facecolor(), edgecolor='none')
        plt.close()
