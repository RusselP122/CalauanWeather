...
def main():
    ...
    for selected_dist, is_wide in targets:
        ...
        ax_map.add_feature(cfeature.LAND, facecolor='#1e293b')
        ax_map.add_feature(cfeature.COASTLINE, edgecolor='#475569', linewidth=1.5)
        ax_map.add_feature(cfeature.BORDERS, edgecolor='#475569', linestyle=':', linewidth=1)
        ax_map.add_feature(cfeature.OCEAN, facecolor='#0f172a')
        ...
        try:
            ...
            if found_geojson_path:
                ...
                ax_map.add_geometries(province_shapely_geometries, crs=ccrs.PlateCarree(), facecolor='none', edgecolor='#475569', linewidth=0.4, alpha=0.4, zorder=3)
        except Exception as province_load_error:
            print(f"Warning: Failed to overlay province boundaries: {province_load_error}")

        # Gridlines and ticks
        gl = ax_map.gridlines(draw_labels=True, linewidth=0.5, color='#334155', alpha=0.8, linestyle='--')
        ...
        ax_map.add_patch(mpatches.Polygon(par_vertices, facecolor='none', edgecolor='#ef4444', 
        ...
        for idx, item in reversed(list(enumerate(chain))):
            ...
            # Current run: 100% opacity white line
        ...
        leg_wind = ax_map.legend(handles=legend_elements, loc='upper left', frameon=True,
                                 facecolor='#1e293b', edgecolor='#334155', labelcolor='#cbd5e1', fontsize=8)
        ...
        for idx, d in enumerate(reversed(chart_data)):
            ...
            # Line color matches the map (white for current, cycle color for previous)
            line_color = '#ffffff' if is_current else cycle_colors[idx]
            text_color = '#ffffff' if is_current else cycle_colors[idx]
        ...
        leg_history.get_title().set_color('#cbd5e1')
        ...
        # Bottom-Right Credit
        credit_text = "Calauan Weather"
        ax_map.text(0.98, 0.02, credit_text, transform=ax_map.transAxes, fontsize=8,
                    verticalalignment='bottom', horizontalalignment='right', weight='bold', color='#cbd5e1')
        ...
        ax_map.set_title(title_text, fontsize=12, weight='bold', pad=10, color='#f8fafc')
        
        # 2. SUBPLOT 1: GENESIS PROBABILITY CHART (Upper Right)
        ax_gen = fig.add_subplot(gs[0, 2])
        ax_gen.set_facecolor('none')
        ax_gen.grid(True, linestyle=':', alpha=0.6, color='#334155')
        ...
        if is_latest_tc:
            ...
            ax_gen.set_ylabel("Forecast Heading", fontsize=10, weight='bold', color='#cbd5e1')
            ax_gen.set_title("Forecast Track Direction Trend", fontsize=11, weight='bold', pad=8, color='#f8fafc')
            ax_gen.tick_params(axis='both', labelsize=9, colors='#cbd5e1')
        else:
            x_names = [d['name'] for d in chart_data]
            ...
            ax_gen.set_ylabel("Genesis Probability (%)", fontsize=10, weight='bold', color='#cbd5e1')
            ax_gen.set_title("Genesis Probability Trend", fontsize=11, weight='bold', pad=8, color='#f8fafc')
            ax_gen.tick_params(axis='both', labelsize=9, colors='#cbd5e1')
        ...
        ax_wind.grid(True, linestyle=':', alpha=0.6, color='#334155')
        ...
        if has_paired_wind:
            ax_wind.plot(x_names, paired, color='#ffffff', linestyle='--', linewidth=1.8, marker='s', markersize=4, label='Paired Track')
            
        ax_wind.set_ylabel("Peak Wind Speed (km/h)", fontsize=10, weight='bold', color='#cbd5e1')
        ax_wind.set_title("Peak Wind Intensity Trend", fontsize=11, weight='bold', pad=8, color='#f8fafc')
        ax_wind.legend(loc='lower left', fontsize=8, facecolor='#1e293b', edgecolor='#334155', labelcolor='#cbd5e1')
        ax_wind.tick_params(axis='both', labelsize=9, colors='#cbd5e1')
